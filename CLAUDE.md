...
pytest、mypy 与 Ruff，以及全量 Web E2E 66/66（Chromium + Mobile Safari；本地补装 WebKit 后与 CI
同口径）。该套件使用受控路由 mock，只能证明 UI 回归。真实 Linux Desktop E2E 为 `3 passed (3.0m)`：配对、WSS
...
已用真实后端数据人工核验工具目录、运行列表与审批卡片渲染。Phase 5/6 收尾已合入 `dev` 分支，
远程 CI 三项 job（Frontend / Web E2E smoke / Backend）全部通过；合并期间暴露的锁文件、测试
环境密钥与沙箱依赖问题及解决方式见[排障记录](docs/troubleshooting.md)。以上均为本地真实运行时
证据，不构成生产部署放行。YuanAI 认证 API 到真实公共 Streamable HTTP MCP 的连接、发现、显式启用、
审批与文档只读调用已有后端证据，但真实外部 MCP 经 Web 控制中心 UI 的完整链路仍未验收。系统性安全
