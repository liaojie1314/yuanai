...
describe('ThinkBlock', () => {
  it('activeState 下头部显示"正在思考…"', () => {
    render(<ThinkBlock content="思考中" active />)
    expect(screen.getByText('正在思考…')).toBeInTheDocument()
  })

  it('非 active 状态显示"已完成思考"与耗时', () => {
    render(<ThinkBlock content="思考完毕" durationMs={3200} defaultOpen />)
    expect(screen.getByText('已完成思考')).toBeInTheDocument()
    expect(screen.getByText('3.2 秒')).toBeInTheDocument()
  })

  it('点击头部切换 open class', () => {
    const { container } = render(<ThinkBlock content="内容" defaultOpen={false} />)
  ...
  it('渲染 toolCalls 列表', () => {
    ...
    render(<ThinkBlock content="" toolCalls={toolCalls} defaultOpen />)
  ...
  it('data-state 属性反映当前状态', () => {
    const { container, rerender } = render(<ThinkBlock content="x" active />)
    expect(container.querySelector('.ch-think-block')?.getAttribute('data-state')).toBe('active')
    rerender(<ThinkBlock content="x" />)
