...
- 语音输入优先使用 Chromium 本地识别；不可用时录制音频并调用后端
  `/api/v1/voice/transcriptions`，转写文本只回填草稿，不自动发送。
- 可配置联网搜索：搜索与思考开关可独立使用；来源保存在消息工具记录中，在思考块的
  `search_web` 详情内安全打开。虚拟列表滚动不会重置已展开的思考或工具详情。
- 媒体任务支持图片、视频、固定 30 秒纯音乐和歌词歌曲；音乐使用消息内原生音频控件播放，
  并可下载后端持久化的 MP3 结果。纯音乐默认使用本机 MusicGen，歌词歌曲使用本机 ACE-Step，
  详见[媒体生成与音乐配置](../../docs/media-generation.md)。
