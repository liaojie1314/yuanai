...
> **当前实现状态（2026-09-03）**：本页是 Phase 6 的目标合同和六个 Wave 的实施路线。
...
> 以及受控 Browser Worker。**本页验收标准尚未达成**：真实外部 MCP 应用链路、完整 Web 控制中心
> 链路、Desktop 取消/重连/撤销、故障注入稳定性和系统性安全测试仍未完成，进入 Phase 7 的许可证仍然阻塞。
...
| 3. MCP 连接与路由         | HTTP + 隔离 stdio Worker 已实现；外部 MCP 应用链路待做             | 阻塞                |
| 4. Desktop 执行节点       | 单个真实 E2E 已通过；取消、重连/重放、撤销仍未覆盖                 | 阻塞                |
...
隔离 stdio Worker、受控 Browser Worker 和外部 MCP 协议探测已有实现或记录，但不能将代码、
协议探测或测试入口等同于生产环境放行。
