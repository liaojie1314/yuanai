...
`autonomy_level`、`is_default`）之上扩展，不新建平行的助理实体：

- `assistant_personas`：`assistant_id`、称呼用户的方式、自称、语气（简洁/耐心/活泼/严谨）、
  回答长度偏好、开场白、拒绝与不确定时的表达方式、禁止话题、语言与称呼习惯
- `assistant_persona_versions`：人格快照与生效时间，便于回滚与"为什么它这么说"的追溯
- 形象与音色字段不在本阶段，见 [Phase 11](./phase-11-personal-digital-twin.md) §6.5

**人格不是权限。** 人格只影响表达方式与主动程度的表现层；
能力边界、工具授权、预算和审批策略仍由 `autonomy_level` 与 Phase 6/8 的授权体系决定，
不得通过人格指令绕过。
...
让用户能拥有"自己的那一个助理"，而不是一个无差别的通用机器人。

### 8.5.1 产品目标

- 用户可创建多个助理，各自有名字、定位、语气和擅长领域（如"工作助理""学习搭子"）
- 人格在多轮、多端、多渠道之间保持一致，不会换个设备就换个人
- 人格可解释、可编辑、可回滚，用户随时能看到"它为什么这样说话"

### 8.5.2 非目标

- 不做虚构人物扮演产品，不提供恋爱/陪伴人设模板
- 不允许人格覆盖安全边界：不能用人格设定让助理放宽拒绝策略或伪装成人类
- 不在本阶段做视觉形象与语音，见 [Phase 11](./phase-11-personal-digital-twin.md) §6.5

### 8.5.3 人格与记忆的绑定

`memories.assistant_id` 已存在，本阶段把它用起来：

| 记忆范围            | 行为                                                   |
| ------------------- | ------------------------------------------------------ |
| `assistant_id` 为空 | 用户级记忆，所有助理共享（如"我叫什么""我在哪个时区"） |
| `assistant_id` 有值 | 该助理私有记忆（如工作助理记住项目上下文）             |

- 检索时按"用户级 + 当前助理私有"合并，不跨助理泄漏私有记忆
- 用户可把某条助理私有记忆**提升为用户级**，或反向降级
- 删除助理时，其私有记忆一并删除并给出明确提示；用户级记忆不受影响

### 8.5.4 人格如何进入上下文

人格片段在 Context Budget（§5.2）中**优先级高于普通记忆但有硬上限**，
避免人格描述挤掉真正有用的上下文：
...
安全策略始终在最前且不可被人格内容覆盖；人格片段超限时按"开场白 → 语气 → 偏好"顺序截断。

### 8.5.5 从使用中演进人格

- 用户明确纠正表达方式（"别这么啰嗦"）时，写入人格候选项而非普通记忆
- 人格变更**一律需要用户确认**，不自动生效 —— 人格是身份，不能被悄悄改写
- 每次变更生成新版本，用户可在人格历史中回滚

### 8.5.6 用户体验

- 助理列表：卡片展示名字、定位、默认模型、自主级别，可设默认助理
- 人格编辑器：表单 + 实时预览（同一句话在当前人格下会怎么回答）
- 人格历史：版本列表、差异对比、一键回滚
- 切换助理时会话侧栏明确提示当前是哪个助理，避免用户误以为在跟同一个说话

### 8.5.7 测试要求

- 人格片段超长时按约定顺序截断，且安全策略永不被截掉
- 助理 A 的私有记忆不出现在助理 B 的检索结果中（跨助理隔离）
- 人格指令试图放宽拒绝策略时无效（人格不提权）
- 人格变更未经确认不生效；回滚后行为随之恢复
- 删除助理后其私有记忆确实删除，用户级记忆仍在

### 8.5.8 验收标准

- 同一用户的两个助理在同一话题上表现出可区分且稳定的语气
- 跨助理记忆隔离有测试证据
- 人格无法绕过任何安全或授权边界，有针对性用例
- 人格任意一次变更都可回滚到上一版本
