...
test('web and desktop jobs upload directly to the prepared GitHub release', () => {
  ...
  assert.match(workflow, /gh release upload "\$RELEASE_TAG"/g)
  assert.match(workflow, /--clobber/g)
  ...
  assert.match(workflow, /Linux release artifacts are incomplete/)
  assert.match(workflow, /Windows release artifacts are incomplete/)
  assert.match(workflow, /macOS release artifacts are incomplete/)
